...
def create_annual_review(user_book_library, user_data_library, year):
    ...
    books_canvas = tk.Canvas(root, width=450, height=300, bg="white", highlightthickness=0)
    ...
    caption_canvas = tk.Canvas(root, width=150, height=300, bg="white", highlightthickness=0)
    caption_canvas.grid(row=1, column=1)
    caption_grid = Grid(40, 50, 12, 1, 20, 20, caption_canvas)

    # Canvas for Pages By Month graph
    pages_canvas = tk.Canvas(root, width=600, height=420, bg="white", highlightthickness=0)
